...
//mode:0,反白显示;1,正常显示
//size:选择字体 12/16/24
void OLED_ShowChar_N(unsigned char x,unsigned char y,unsigned char chr,unsigned char size,unsigned char mode)
{      			    
	unsigned char temp,t,t1;
	unsigned char y0=y;
	unsigned char csize=(size/8+((size%8)?1:0))*(size/2);		//得到字体一个字符对应点阵集所占的字节数
...
//size1:字体大小 
//*chr:字符串起始地址 
void OLED_ShowString_N(unsigned char x,unsigned char y,const unsigned char *p,unsigned char size,unsigned char mode)
{	
    while((*p<='~')&&(*p>=' '))//判断是不是非法字符!
    {       
        if(x>(128-(size/2))){x=0;y+=size;}     //换行
        if(y>(64-size)){y=x=0;OLED_Clear();}
        OLED_ShowChar_N(x,y,*p,size,mode);	 
        x+=size/2;                             //换列
        p++;
    }  
}
...
{ 
    ...
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_9 | GPIO_PIN_8, GPIO_PIN_SET); 	//PB8,PB9 输出高

    GPIO_InitStructure.Pin = GPIO_PIN_11 | GPIO_PIN_12;
    GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP ;   //推挽输出
    GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(GPIOD, &GPIO_InitStructure);
    HAL_GPIO_WritePin(GPIOD, GPIO_PIN_11 | GPIO_PIN_12, GPIO_PIN_SET); 	//PD11,PD12 输出高
