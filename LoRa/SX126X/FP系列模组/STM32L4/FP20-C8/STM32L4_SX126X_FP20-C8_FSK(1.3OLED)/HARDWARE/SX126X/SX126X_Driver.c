...
{
    if((G_LoRaConfig.LoRa_Freq > 920000000) || (G_LoRaConfig.LoRa_Freq < 410000000))
...
/**
  * @简介：发射功率设置，该函数配置是结合硬件LSD4RF-2R717N40\LSD4RF-2R714N10\LSD4RF-2R722N20\考虑功耗及功率最优设置； 
  * @参数：Power输入值范围：-3~22；
  * @返回值:
  */
void SX126xSetTxParams( int8_t power, RadioRampTimes_t rampTime )
...
void SX126xSetLoraModulationParams(void)
{
    ...
    else
    {
	 ...
	 if( ( G_LoRaConfig.SpreadingFactor == LORA_SF11 ) || ( G_LoRaConfig.SpreadingFactor == LORA_SF12 ) )
     {
			 //采集分辨率调准，主要解决低速率大数据包下丢包问题
        SX126xWriteRegister(0x889, SX126xReadRegister(0x889) & 0xfB);
     } 
...
{
if((G_LoRaConfig.LoRa_Freq < 410000000) || (G_LoRaConfig.LoRa_Freq > 810000000))
        return false;

G_LoRaConfig.BandWidth = (RadioLoRaBandwidths_t)(G_LoRaConfig.BandWidth & 0x0F);
    if(G_LoRaConfig.BandWidth > 0x0A)
        return false;
  //计算BandWidth
...
tSX126xError SX126x_Lora_init(void)
{
    ...
    SX126xSetStandby(STDBY_RC);    //切换到LoRamode，standby状态
		...
		{
				SX126xSetDio3AsTcxoCtrl(TCXO_CTRL_2_7V, 6400);
		}
...
//    SX126xSetLoraModulationParams( );   //SF、BW、CR、LOWDATARATEOPTIMIZE
    SX126xSetLoraPacketParams( );  //PreambleLength、HeaderType、PayloadLength、CrcMode、InvertIQ
    SX126xSetBufferBaseAddress( 0x00, 0x00 );
    SX126xSendPayload( data, G_LoRaConfig.PayloadLength, 0x200000 );//Timeout disable, Tx Single mode
}
/**
  * @简介  :发送数据包
...
/*************************Best CAD Setting****************************/
/*  SF      cadDetMin    cadDetPeak     cadSymbolNum                 */  
/*  SF7       10             22            2 symbol                  */ 
/*  SF8       10             22            2 symbol                  */ 
/*  SF9       10             23            4 symbol                  */ 
/*  SF10      10             24            4 symbol                  */ 
...
void SX126X_CADinit(void)
{
  SX126xSetStandby(STDBY_RC);
	SX126xSetPacketType( PACKET_TYPE_LORA );  //设置成LORA模式
	SX126xSetRegulatorMode( USE_DCDC );
	SX126xSetCadParams( LORA_CAD_04_SYMBOL, 22, 10, LORA_CAD_ONLY, 0xff );//
	SX126XWriteRxTx(false);
...
void SX126X_WOR_Exit(void)
{
    ...
    SX126xSetDioIrqParams( IRQ_RX_DONE|IRQ_CRC_ERROR, 
	  ...
	  SX126xSetStopRxTimerOnPreambleDetect( false );
    SX126xSetLoRaSymbNumTimeout(0);
	  G_LoRaConfig.PreambleLength=0xffff; //这里接收的前导码一定要等于或大于唤醒包时实际前导码长，所以设置成最大值
//	  SX126xSetLoraModulationParams( );   //SF、BW、CR、LOWDATARATEOPTIMIZE
    SX126xSetLoraPacketParams( );  //PreambleLength、HeaderType、PayloadLength、CrcMode、InvertIQ设置
    SX126xSetBufferBaseAddress( 0x00, 0x00 );
    SX126xSetRx( 0xffffff ); // Rx Continuout
...
void SX126X_Awake(uint8_t*cbuf, uint16_t Preamble_Length)
{
    ...
    SX126XWriteRxTx(true);
	  ...
	  SX126xSetLoraPacketParams( );//PreambleLength、HeaderType、PayloadLength、CrcMode、InvertIQ设置
    SX126xSetBufferBaseAddress( 0x00, 0x00 );
    SX126xSendPayload( cbuf, G_LoRaConfig.PayloadLength, 0x000000);//Timeout disable, Tx Single mode
}
