/**
  ...
  * 时间   ：   15-Aug-2018
  * 文件描述：
  *    A模块先进入接收状态，去读写当前环境中的RSSI值，当RSSI值小于域值说明存在干扰信号（信道不被占用）发送数据包给B模块，否则不发送数据TX计数加1，B模块收到数据后，B模块将数据回传给A
...
int16_t Threshold_Rssi = (-90);//设置的信道中的RSSI阈值（这个值需要根据测试环境中的RSSI去定，一般的环境中的RSSI为-100dBm至-115dBm）
int16_t Rssi_Value = 0; //当前信道环境中RSSI
...
int main(void)
{
    ...
    OLED_ShowString(0, 24, "Current_RSSI:", 12);
    ...
    while (1)
    {
			 if(Rssi_Value < 0)
				 {
           OLED_ShowString(84, 24, "-", 12);
           Rssi_Value = -Rssi_Value;
           OLED_ShowNum(90, 24, Rssi_Value, 4, 12);
         }
			else
				 {
					OLED_ShowString(84, 24, " ", 12);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    ...
    if(DIO1_GetState() == GPIO_PIN_SET)
    {
        ...
        {	         					
            ...
            OLED_ShowNum(12, 48, T_Cnt, 3, 12); //显示已发送数据包数量
            DIO1_EnableInterrupt();
            SX126X_StartRx();//进入接收模式，准备接收数据
        }
//				else if((flag & IRQ_RX_DONE) == IRQ_RX_DONE)//产生RxDone，有接收到数据
        else if((flag & (IRQ_RX_DONE | IRQ_CRC_ERROR)) == IRQ_RX_DONE)//产生RxDone，有接收到数据
        {
            ...
            OLED_ShowNum(48, 48, R_Cnt, 3, 12);//显示接收到数据包数量
...
{
		Rssi_Value=SX126xGetRssiInst();
		if(Rssi_Value<Threshold_Rssi)
      {
